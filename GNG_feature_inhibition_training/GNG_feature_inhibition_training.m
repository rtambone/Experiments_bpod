function GNG_feature_inhibition_training
global BpodSystem
...
for x = 1:Data.nTrials
    if TrialTypes(x) == 1 % odor
        if ~isnan(Data.RawEvents.Trial{x}.States.TimeOutState(1))
            Outcomes(x) = -1; % licked and punished
        else 
            Outcomes(x) = 2; % not licked nothin
        end
    elseif TrialTypes(x) == 2 %
        if ~isnan(Data.RawEvents.Trial{x}.States.Reward(1))
            Outcomes(x) = 1; % licked and punished
        else 
            Outcomes(x) = 3; % not licked nothing
        end
    end
end
